      ...
      <form @submit.prevent="handleFormSubmit" class="space-y-6">
        ...
        <div>
          ...
          <input
            ...
            placeholder="https://ticketing.com/..."
            class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-primary focus:border-primary sm:text-sm"
          >
        </div>
...
const filteredEvents = computed(() => {
  ...
  if (statusFilter.value) {
    ...
    if (statusFilter.value === 'upcoming') {
      ...
      events = events.filter(event => event.is_sold_out);
    }
  }
...
const stats = computed(() => {
  ...
  return {
    upcoming: props.events.filter(e => new Date(e.event_date) >= now).length,
    past: props.events.filter(e => new Date(e.event_date) < now).length,
    soldOut: props.events.filter(e => e.is_sold_out).length
  }
})
...
const handleFormSubmit = () => {
  ...
  Object.keys(form.value).forEach(key => {
    ...
    } else if (key === 'is_sold_out' || key === 'is_visible') {
  ...
  if (editingEvent.value) {
    ...
    router.post(`/admin/events/${editingEvent.value.id}`, formData, {
      preserveScroll: true,
      onSuccess: (page) => {
        cancelForm()
